...
Next, import the content and tests:
```
./import.sh
```
...
The test files, as well as the `test-config.json` are located inside the `tests` directory.  
When rerunning the script, no existing files will be overwritten. To reload 
content and tests, delete the corresponding files before running the `import.sh` script.
